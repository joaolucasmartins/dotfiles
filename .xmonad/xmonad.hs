...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm              , xK_u     ), mconcat $  spawn "xmonad --restart" : [])
    , ((modm              , xK_t     ), mconcat $ spawn "rm ~/.xmonad/xmonad.errors && xmonad --recompile" : (runOnWorkspaces $ \ww -> return $ ww { W.layout = XMonad.layoutHook conf}) :  broadcastMessage ReleaseResources : [])
-- mconcat $ fmap (\w -> updateLayout w $ Just (XMonad.layoutHook conf)) myWorkspaces
...
topBarTheme = def
    ...
    , inactiveTextColor     = color2
    , activeBorderColor     = color3
    , activeColor           = color4
    , activeTextColor       = active
    , urgentBorderColor     = color5
