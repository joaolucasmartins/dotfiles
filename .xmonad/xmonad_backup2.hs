...
myLogHook dbus = def
    ...
    , ppCurrent = wrap ("%{F" ++ color7 ++ "} ") " %{F-}" . wrap "|" " |"
    , ppVisible = wrap ("%{F" ++ color7 ++ "} ") " %{F-}"
    , ppUrgent = wrap ("%{F" ++ red ++ "} ") " %{F-}"
    , ppHidden = wrap ("%{F" ++ color5 ++ "} ") " %{F-}"
    , ppHiddenNoWindows = wrap ("%{F" ++ color5 ++ "} ") " %{F-}"
    , ppTitle = wrap ("%{F" ++ color6 ++ "} ") " %{F-}"
    }
...
myStartupHook = do
    spawn "killall polybar && wait; polybar config"
